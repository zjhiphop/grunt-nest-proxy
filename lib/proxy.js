    ...
    request: function(req, res, next) {
        ...
        proxies && proxies.forEach(function(proxy) {
            if (!proxied && req && Util.matchContext(proxy.config.context, req.url)) {
                ...
                proxy.r.get(target).pipe(res);


                console.log('Proxied request: ' + source + ' -> ' + target + '\n' + JSON.stringify(req.headers, true, 2));

                console.log("Start to request : header, ", req.headers, "  Source URL: ", req.originalUrl, ' target:', target);
            }
        });
